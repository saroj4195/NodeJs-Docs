...
| 5 | `min-rates` | Fetches 60-day rate snapshot for availability calendar display. |
| 6 | `locale-details` | Fetches applicable GST or service taxes as per room type and rate plan. |
| 7 | `paid-services` | Lists any extra/optional paid services available for guests. |
| 8 | `connected-ota-rates` | Compares property pricing vs. OTA pricing to display offer/value insights. |
...
| 13 | `bookings-new` | This API is use for creat a new reservation. |
| 14 | `uniqueVisitors` | Tracks unique visitor count for analytics. |
| 15 | `get-be-version-data` | Fetches version-specific data for the Booking Engine. |
| 16 | `get-hotel-banner-list` | Retrieves a list of banners for the hotel. |
| 17 | `hotel-amenities` | Fetches hotel amenities. |
| 18 | `footer/payment-icon` | Fetches payment icons for the footer. |
| 19 | `currency` | Fetches available currencies. |
| 20 | `check-private-coupon` | Checks the validity of a private coupon. |
| 21 | `download-invoice-details` | Downloads invoice details. |
| 22 | `get-all-country` | Fetches all countries. |
| 23 | `get-all-states` | Fetches all states for a given country. |
| 24 | `get-all-city` | Fetches all cities for a given state. |
